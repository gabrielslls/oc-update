...
```bash
# 一键修复（推荐）
oc-update config --fix

# 或手动修复
cd ~/.cache/opencode
bun add oh-my-openagent@<版本号>   # 使用精确版本，不要用 @latest

# 重启 OpenCode
```

`oc-update update` 和 `oc-update auto` 现在会自动同步缓存，无需手动干预。
如需跳过自动缓存同步，设置环境变量 `OC_NO_CACHE_SYNC=1`。
